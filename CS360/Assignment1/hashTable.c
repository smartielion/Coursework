/* Casey Gilray
This document is being reused from CS261 Assignment 5, also written by Casey Gilray

Carries all the information needed to make a hashtable, place values in, then clean/clear it as nessicary
*/
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
void rehash(hashtable* table){
	...
	for (int i = 0;i<table->size;i++){
		node* current=table->table[i];
		while (current != NULL){
			insert(new,current->key, current->value);
			prev = current;
			current = current->next;
			free(prev);
		}
...
int insert(hashtable* table,char* key,void* value){
	int hashVal = hash(key);
